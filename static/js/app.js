const fields = [
    "Time", "V1", "V2", "V3", "V4", "V5", "V6", "V7", "V8", "V9",
    "V10", "V11", "V12", "V13", "V14", "V15", "V16", "V17", "V18",
    "V19", "V20", "V21", "V22", "V23", "V24", "V25", "V26", "V27",
    "V28", "Amount"
];

const samples = {
    normal: {"Time":0.0,"V1":-1.3598071337,"V2":-0.0727811733,"V3":2.536346738,"V4":1.3781552243,"V5":-0.3383207699,"V6":0.4623877778,"V7":0.2395985541,"V8":0.0986979013,"V9":0.3637869696,"V10":0.090794172,"V11":-0.5515995333,"V12":-0.6178008558,"V13":-0.9913898472,"V14":-0.3111693537,"V15":1.4681769721,"V16":-0.4704005253,"V17":0.2079712419,"V18":0.0257905802,"V19":0.4039929603,"V20":0.2514120982,"V21":-0.0183067779,"V22":0.2778375756,"V23":-0.1104739102,"V24":0.0669280749,"V25":0.1285393583,"V26":-0.1891148439,"V27":0.1335583767,"V28":-0.0210530535,"Amount":149.62},
    fraud: {"Time":406.0,"V1":-2.3122265423,"V2":1.9519920106,"V3":-1.6098507323,"V4":3.9979055875,"V5":-0.5221878647,"V6":-1.4265453192,"V7":-2.5373873062,"V8":1.3916572483,"V9":-2.7700892772,"V10":-2.7722721447,"V11":3.2020332071,"V12":-2.8999073885,"V13":-0.5952218813,"V14":-4.2892537824,"V15":0.3897241203,"V16":-1.1407471798,"V17":-2.8300556745,"V18":-0.0168224682,"V19":0.416955705,"V20":0.1269105591,"V21":0.5172323709,"V22":-0.0350493686,"V23":-0.4652110762,"V24":0.3201981985,"V25":0.0445191675,"V26":0.1778397983,"V27":0.2611450026,"V28":-0.1432758747,"Amount":0.0}
};

const fieldGrid = document.getElementById("fieldGrid");
const form = document.getElementById("transactionForm");
const resultTitle = document.getElementById("resultTitle");
const resultBadge = document.getElementById("resultBadge");
const fraudProbability = document.getElementById("fraudProbability");
const predictionConfidence = document.getElementById("predictionConfidence");
const thresholdUsed = document.getElementById("thresholdUsed");
const errorMessage = document.getElementById("errorMessage");

fields.forEach((field) => {
    const label = document.createElement("label");
    label.textContent = field;

    const input = document.createElement("input");
    input.type = "number";
    input.step = "any";
    input.name = field;
    input.required = true;

    label.appendChild(input);
    fieldGrid.appendChild(label);
});

function loadSample(sample) {
    fields.forEach((field) => {
        form.elements[field].value = sample[field];
    });
    errorMessage.textContent = "";
}

function formatPercent(value) {
    return `${(Number(value) * 100).toFixed(2)}%`;
}

document.getElementById("normalSample").addEventListener("click", () => loadSample(samples.normal));
document.getElementById("fraudSample").addEventListener("click", () => loadSample(samples.fraud));

form.addEventListener("submit", async (event) => {
    event.preventDefault();
    errorMessage.textContent = "";
    resultTitle.textContent = "Checking...";
    resultBadge.className = "badge";
    resultBadge.textContent = "Model running";

    const payload = {};
    fields.forEach((field) => {
        payload[field] = Number(form.elements[field].value);
    });

    try {
        const response = await fetch("/check-transaction", {
            method: "POST",
            headers: {"Content-Type": "application/json"},
            body: JSON.stringify(payload)
        });

        if (!response.ok) {
            throw new Error("Prediction request failed");
        }

        const result = await response.json();
        resultTitle.textContent = result.fraud ? "Fraud Detected" : "Transaction Looks Normal";
        resultBadge.className = result.fraud ? "badge danger" : "badge safe";
        resultBadge.textContent = result.fraud ? "High risk" : "Low risk";
        fraudProbability.textContent = formatPercent(result.fraud_probability);
        predictionConfidence.textContent = formatPercent(result.prediction_confidence);
        thresholdUsed.textContent = formatPercent(result.threshold_used);
    } catch (error) {
        resultTitle.textContent = "Could not check transaction";
        resultBadge.className = "badge";
        resultBadge.textContent = "Request failed";
        errorMessage.textContent = error.message;
    }
});

loadSample(samples.normal);
